...
const HistoryList: React.FC<HistoryListProps> = ({ items, onPlay, onDelete, lang }) => {
  ...
  const handleDownload = (item: HistoryItem) => {
    if (!item.audioUrl || item.audioUrl.startsWith('local:')) return;

    // Create a safe filename from the text
    const safeName = item.text.slice(0, 20).replace(/[^a-z0-9]/gi, '_').toLowerCase();
    const filename = `VNAudioPro_${item.voiceName}_${safeName}_${item.id}.wav`;
          ...
          {filteredAndSortedItems.map((item) => (
            <div key={item.id} className="group glass-card p-3 md:p-4 rounded-2xl flex items-center justify-between gap-4 hover:shadow-lg hover:border-indigo-200 transition-all border border-transparent">
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                ...
                {/* Download Button - Only for Cloud Voices */}
                {!item.audioUrl.startsWith('local:') && (
                  <button 
                    onClick={() => handleDownload(item)}
                    className="p-1.5 md:p-2 bg-white text-emerald-600 border border-emerald-100 rounded-lg md:rounded-xl hover:bg-emerald-50 transition-all active:scale-90"
                    title={lang === 'vi' ? 'Tải xuống' : 'Download'}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4-4m0 0L8 8m4-4v12" />
                    </svg>
                  </button>
                )}
