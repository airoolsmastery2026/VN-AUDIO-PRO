  ...
  useEffect(() => {
    ...
    if (savedHistory) {
      try {
        ...
        setHistory(parsed.map((item: HistoryItem) => ({ ...item, audioUrl: '' })));
      } catch (e) {}
    }
  }, []);
    ...
    <div className="min-h-screen bg-[#f1f5f9] pb-24 animate-in fade-in duration-700">
      ...
      <main className="max-w-[1440px] mx-auto px-6 md:px-12 py-10">
        {activeTab === 'pricing' ? (
          ...
          <BatchStudio tasks={[]} onAddBatchTasks={() => {}} onUpdateTask={() => {}} onRemoveTask={() => {}} onClearTasks={() => {}} onStartBatch={() => {}} onStopBatch={() => {}} onDownloadAll={() => {}} isProcessing={false} languages={LANGUAGES} autoDownload={false} onToggleAutoDownload={() => {}} lang={lang} />
        ) : (
            ...
            <div className="lg:col-span-8 space-y-8">
              ...
              <section className="space-y-8">
                <div className="flex items-center justify-between px-2">
                  ...
                  <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest bg-indigo-50 px-3 py-1 rounded-full">Pro Voices</span>
                </div>
            ...
            <div className="lg:col-span-4">
              <section className="bg-white rounded-[2.5rem] p-8 shadow-2xl shadow-slate-200/50 border border-slate-100 flex flex-col h-[700px] lg:h-[calc(100vh-160px)] lg:sticky lg:top-32">
                ...
                <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                  <HistoryList lang={lang} items={history} onPlay={(item) => { if (audioRef.current) { audioRef.current.src = item.audioUrl; audioRef.current.play(); } }} onDelete={(id) => setHistory(prev => prev.filter(h => h.id !== id))} />
                </div>
              </section>
            </div>
          </div>
